...
class sendReservationApprovedNotifications extends Job implements ShouldQueue
{
    ...
    {
        $this->sendReservationWasApprovedToCustomer($this->reservation);
     ...
     */
    public function sendReservationWasApprovedToCustomer(Reservation $reservation)
    {
        // Customer Reservation Created Notification
        $user = User::whereId($reservation->user_id)->first();

        Mail::send(
            'emails/cart/customer/approved',
            [
                'user' => $user,
            ...
            function ($message) use ($user) {
                ...
                $message->to($user->email, $user->name)->subject(trans('Reservation was Approved'));
            }
        );
    }
    ...
    public function sendPushNotificationToCustomer(Reservation $reservation)
    {
        ...
        $message = trans('cart.notifications.reservation.approved', ['id' => $reservation->id]);
